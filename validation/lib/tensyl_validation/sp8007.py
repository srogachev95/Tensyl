...
written so the validation report can show where Tensyl agrees, where it
diverges, and which modeling assumption causes the divergence.
"""
...
def default_reconciliation_cases() -> tuple[SP8007ComparisonCase, ...]:
    ...
    return (
        ...
        SP8007ComparisonCase(
            ...
            in_plane_inertia=low_in_plane,
            eccentricity=0.0,
            note="Isogrid limit closest to SP-8007 Eqs. 92-98 as written.",
        ),
        SP8007ComparisonCase(
            ...
            in_plane_inertia=low_in_plane,
            note=(
                "Isogrid case isolating eccentric axial stiffness, which SP-8007 "
                "Eqs. 97-98 do not show explicitly."
            ),
        ),
    )
...
def sp8007_reference_coefficients(case: SP8007ComparisonCase) -> dict[CoefficientName, float]:
    """Compute SP-8007 coefficients for a reconciliation case."""
            ...
            rows.append(
                {
                    ...
                    "tensyl": actual,
                    "sp8007_as_written": expected,
                    "delta": delta,
                    "relative_delta": delta / scale,
                    "abs_relative_delta": abs(delta) / scale,
                    "case_note": case.note,
                }
            )
    return rows
...
def summary_payload(rows: list[dict[str, Any]], sweep: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for case_name in sorted({str(row["case_name"]) for row in rows}):
        ...
        worst = max(case_rows, key=lambda row: float(row["abs_relative_delta"]))
        worst_by_case.append(
            {
                "case_name": case_name,
                "model": worst["model"],
                "worst_coefficient": worst["coefficient"],
                "worst_abs_relative_delta": worst["abs_relative_delta"],
            }
        )
    return {
        "schema_version": "tensyl.validation.sp8007-reconciliation-summary.v1",
        ...
        "interpretation": [
            ...
            "Membrane and coupling terms are mostly convention checks in these cases.",
            (
                "Large bending differences track retained in-plane member bending and "
                "eccentric axial stiffness."
            ),
            (
                "A disagreement can mean a Tensyl bug, an SP-8007 limitation, a "
                "convention mismatch, or two valid model choices."
            ),
        ],
